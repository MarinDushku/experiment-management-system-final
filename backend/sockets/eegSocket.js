...
const handleEEGEvents = (io, connectedDevices, devicePairs) => {
  // Store active EEG streaming sessions
  const eegSessions = new Map();

  io.on('connection', (socket) => {
    console.log(`EEG socket connected: ${socket.user.username} (${socket.id})`);

    // EEG streaming control
    socket.on('eeg-stream-start', (data) => {
      const { experimentId, streamConfig = {} } = data;

      console.log(`EEG stream start requested by ${socket.user.username} for experiment ${experimentId}`);
      ...
      socket.to('admin_devices').emit('eeg-stream-started', {
        sessionId,
        experimentId,
        adminUser: socket.user.username,
        timestamp: Date.now()
      });
    });
    ...
    socket.on('eeg-stream-stop', (data) => {
      ...
      console.log(`EEG stream stop requested by ${socket.user.username} for experiment ${experimentId}`);
    ...
    socket.on('disconnect', () => {
      console.log(`EEG socket disconnected: ${socket.user.username} (${socket.id})`);
